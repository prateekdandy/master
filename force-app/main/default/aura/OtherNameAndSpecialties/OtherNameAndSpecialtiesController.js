({
    ...
    enableAddContact: function(component, event, helper){
        component.set('v.contactSpecialty', event.getSource().get('v.value'));
        if(component.get('v.contactLastName') !== undefined && component.get('v.contactFirstName') !== undefined &&
            ...
            for(var i=0;i<inputs.length;++i){
                if(inputs[i].id === fileShareId && inputs[i].ableToAdd === 'false'){
                    ...
                    component.get('v.contactFirstName'), fileShareId, component.get('v.contactSpecialty'), '', 'true', 'false', '');
                }
            }
        }else{
            if(event.getSource().get('v.value') !== ''){
                helper.updateSpecialtyForContact(component, event, helper);
            }
        }
    },
