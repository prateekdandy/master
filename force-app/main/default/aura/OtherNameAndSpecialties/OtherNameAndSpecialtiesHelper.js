({
    checkForContactsAlreadyCreated: function(component, event, helper){
        ...
        action.setParams({contactRole: 'Other Dentist', idOpportunity: component.get('v.oppRecordId')});
                ...
                if(response.getReturnValue().length>0){
                    ...
                    for(var i=0; i<contactRolesAlreadyCreated.length; ++i){
                        inputs.push({'lastName': contactRolesAlreadyCreated[i].LastName, 'firstName': contactRolesAlreadyCreated[i].FirstName,'id': 'fileShare' + component.get('v.fileShareIncremental'),
                            'specialty': contactRolesAlreadyCreated[i].Specialty__c, 'recordToInsert': 'false', 'isSaved': 'true', 'ableToAdd': 'false', 'recordToUpdate': 'false', 'idContact': contactRolesAlreadyCreated[i].Id});
                    ...
                    component.set('v.fileShares', {'lastName': '', 'firstName': '','id': 'fileShare' + component.get('v.fileShareIncremental'),
                             'specialty': '', 'recordToInsert': '', 'isSaved': 'false', 'ableToAdd': 'false', 'recordToUpdate': 'false', 'idContact': ''});
                    component.set('v.fileShareIncremental', component.get('v.fileShareIncremental') + 1);
                }
    ...
    buildRecordToSave: function(component, event, helper){
        ...
        if(component.get('v.contactOption') !== undefined){
            this.saveRecord(component, event, helper, component.get('v.contactOption.LastName'),
                ...
                component.get('v.contactOption.Specialty__c'), 'false', 'false', 'false', component.get('v.contactOption.Id'));
        }else{
            this.saveRecord(component, event, helper, component.get('v.contactLastName'),
                component.get('v.contactFirstName'), component.get('v.fileShareId'),component.get('v.contactSpecialty'), 'true', 'false', 'false', '');
        }
    },

    saveRecord: function(component, event, helper, lastName, firstName, idToUpdate, specialty, recordInserted, ableToAdd, recordToUpdate, idContact){
        var isSaved = 'true';
        if(ableToAdd === 'true'){
            isSaved = 'false';
        }
    ...
    updateSpecialtyForContact: function(component, event, helper){
        var inputs = component.get('v.fileShares');
